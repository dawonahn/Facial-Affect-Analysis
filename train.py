...
# f = open('./result/training.txt', 'a') #save the result
...
for epoch in range(20):  # loop over the dataset multiple times
    ...
    for i, data in tqdm(enumerate(training_dataloader)):
        ...
        arousal = labels[:, 0].cuda(DEVICE)
        valence = labels[:, 1].cuda(DEVICE)

        # zero the parameter gradients
        optimizer.zero_grad()

        # forward + backward + optimize
        outputs = model(inputs.cuda(DEVICE))
...
# f2 = open('./result/final.txt', 'a')
# f2.write(f'{test_arousal:.4}\t{test_valence}\n')

# Save the model
# torch.save(model,'./result/model03.pth' )
